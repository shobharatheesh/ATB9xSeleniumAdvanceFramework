...
import com.testingacademy.pages.pageFactory.DashboardPage_PF;
...
public class TestVWOLogin08_Improved_log4j_POM_PropertyReader_DriverManager_RetryListeners extends CommonToAllTest {

    private static final Logger logger = LogManager.getLogger(TestVWOLogin08_Improved_log4j_POM_PropertyReader_DriverManager_RetryListeners.class);
    ...
    public void testLoginPositiveFailedVWO() {
        System.out.println("Start of test case");
        logger.info("Starting the Positive Failed Testcases VWO | Verify that valid creds and dashboard page is loaded");

        LoginPage login = new LoginPage(DriverManager.getDriver());
        login.loginToVWOvalidCreds(PropertiesReader.readKey("valid_username"), PropertiesReader.readKey("valid_password"));

        DashboardPage_PF dashboard = new DashboardPage_PF(DriverManager.getDriver());
        String usernameLoggedIn = dashboard.loggedInUserName();

        System.out.println("End of test case");
        logger.info("Ending and asserting the Positive Failed Testcases VWO");

        // assertj assertion
        assertThat(usernameLoggedIn).isNotBlank().isNotEmpty().isNotNull();

        // testng asserting
        Assert.assertEquals(usernameLoggedIn, PropertiesReader.readKey("Pramod"));
        System.out.println(usernameLoggedIn);
    }
